  ...
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          remark: [
            { required: true, message: '请填写直播备注', trigger: 'change' }
          ],
          number: [
    ...
    methods: {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            param.append('valid', this.ruleForm.valid);
            param.append('redirectUrl', this.ruleForm.redirectUrl);
            axios({
              method:"POST",
              url:"http://"+this.baseUrl+"/flowPool/admin/liveRadio/update",
              headers:{
      ...
      getProductList(){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/liveRadio/get",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': this.$store.state.userToken
          },
