<template>
  <div class="content">
    <el-breadcrumb class="fs-16" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item>活动Banner管理</el-breadcrumb-item>
    </el-breadcrumb>
  ...
  export default {
    methods: {
      ...
      getProductList(data1,data2,data3){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/activityBanner/list",
          headers:{
      ...
      prohibitBtn(row){
        ...
        let enabled=(row.enabled==true)?1:0;
        let id=row.id;
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/activityBanner/stopOrDelete",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': this.$store.state.userToken
          },
