<template>
  <div class="content">
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="ruleForm">
        ...
        <div class="jiegou" v-if="ruleForm.mode==item.module" style="width: 700px">
          <el-form-item :label="item.nameText" prop="nameText">
            ...
            <!--<div v-else-if="ruleForm.count==13">-->
              ...
              <!--&lt;!&ndash;<el-select v-if="index==5" v-model="item.valueId" placeholder="请选择" @change="changeSelect3($event,index,selectList)">&ndash;&gt;-->
              <!--&lt;!&ndash;<el-option&ndash;&gt;-->
              <!--&lt;!&ndash;v-for="item in selectList"&ndash;&gt;-->
      ...
      <el-form-item label-width="260px" style="width: 740px;background-color: #fff;padding-bottom: 60px;">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loaderModelTable() {
        axios({
          ...
          url:"http://"+this.baseUrl+"/flowPool/admin/productManage/loaderModelTable",
          headers:{
      ...
      handleClick(key,id,num) {
        ...
        axios({
          method:"post",
          url:"http://"+this.baseUrl+"/flowPool/admin/productManage/loaderModel",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': this.$store.state.userToken
          },
      ...
      getRiskList(){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/productManage/getRiskList",
          headers:{
      ...
      getAmountList(){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/productManage/getAmountList",
          headers:{
            'Content-Type':'application/json',
            'Authorization': this.$store.state.userToken
          },
      ...
      getProductList(){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/productManage/list",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': this.$store.state.userToken
          },
      ...
      getBorrowingProductList(){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/product/getList",
          headers:{
            'Content-Type':'application/json',
            'Authorization': this.$store.state.userToken
          }
      ...
      submitForm(module,moduleName){
        ...
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/productManage/saveModel",
          headers:{
            'Content-Type':'application/json',
            'Authorization': this.$store.state.userToken
          },
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            ...
            this.$router.push('/productConfiguration');
    ...
    mounted:function () {
      this.id=1;
      this.merchantId=1;
      this.handleClick('VerificatSMS','验证码短信');
