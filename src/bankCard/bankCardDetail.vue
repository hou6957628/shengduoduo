<template>
  <div class="content">
    ...
    <div class="bankBox">
      <p><span>编号：</span>{{(bankCard.bankCode)?bankCard.bankCode:'----'}}</p>
      ...
      <p><span>用户姓名：</span>{{bankCard.userName}}</p>
  ...
  export default {
    methods: {
      ...
      getBankCardById(id){
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/flowPool/admin/bank/getBankCardInfoById",
          headers:{
    ...
    mounted:function () {
      this.id=this.$store.state.bankCard;
